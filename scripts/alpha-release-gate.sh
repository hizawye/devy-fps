...
version_tag="${1:-v0.2.0-alpha}"
config_path="${2:-${repo_root}/config/server_test.json}"
preset="${3:-debug-vcpkg}"
clients="${4:-8}"
scenario_seconds="${5:-8}"
endurance_minutes="${6:-480}"
out_dir="${7:-${repo_root}/artifacts/releases/alpha-gate/$(date +%Y%m%d-%H%M%S)}"
run_endurance="${8:-1}"
from_ref="${9:-}"
...
docs_required=(
  "${repo_root}/docs/releases/alpha-bug-bash-checklist.md"
  "${repo_root}/docs/releases/alpha-known-issues.md"
  "${repo_root}/docs/releases/alpha-acceptance-checklist.md"
  "${repo_root}/docs/releases/alpha-release-tag-flow.md"
)
...
if [[ "${overall}" == "pass" ]]; then
  echo "Alpha release gate passed. Artifacts: ${out_dir}"
  exit 0
fi

echo "Alpha release gate failed. See ${summary_log}" >&2
exit 1
